...
#   - Tests a :one association with inverse one association with foreign key.
RSpec.describe Stannum::Associations::One do
  ...
  shared_examples 'should not change the association' do
    ...
    it 'should not change the inverse foreign key' do
      expect { update_association }.not_to(change { entity.dungeon&.boss_id })
    end
  end
  ...
  shared_examples 'should update the association' do
    ...
    it 'should clear the previous inverse association foreign key' do
      next unless defined?(dungeon)
    ...
    it 'should set the new inverse foreign key' do
      expect { update_association }
        .to change(new_dungeon, :boss_id)
        .to be == entity.id
    end
  end

  let(:described_class) { Spec::Boss }
  let(:attributes)      { { id: 0, challenge: 20, name: 'Lich Lord' } }
  let(:associations)    { {} }
  let(:new_dungeon_boss) do
    Spec::Boss.new(id: 2, challenge: 1, name: 'Skellington')
  end
  let(:new_dungeon) do
  ...
  example_class 'Spec::Dungeon' do |klass|
    ...
    klass.association :one, :boss, class_name: 'Spec::Boss', foreign_key: true
  end
  ...
  describe '#properties=' do
    let(:value) { { id: entity.id, dungeon: new_dungeon } }
    let(:expected) do
      {
        'id'        => entity.id,
        'challenge' => nil,
