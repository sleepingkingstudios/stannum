...
RSpec.describe Stannum::Constraints::Delegator do
  ...
  describe '#update_errors_for' do
    let(:actual) { Object.new.freeze }
    let(:errors) { Stannum::Errors.new }

    it { expect(constraint).not_to respond_to(:update_errors_for) }

    it 'should define the private method' do
      expect(constraint).to respond_to(:update_errors_for, true)
        .with(0).arguments
        .and_keywords(:actual, :errors)
    end

    it 'should delegate to #receiver' do
      allow(receiver).to receive(:update_errors_for)

      constraint.send(:update_errors_for, actual: actual, errors: errors)

      expect(receiver)
        .to have_received(:update_errors_for)
        .with(actual: actual, errors: errors)
    end
  end

  describe '#update_negated_errors_for' do
    let(:actual) { Object.new.freeze }
    let(:errors) { Stannum::Errors.new }

    it { expect(constraint).not_to respond_to(:update_negated_errors_for) }

    it 'should define the private method' do
      expect(constraint).to respond_to(:update_negated_errors_for, true)
        .with(0).arguments
        .and_keywords(:actual, :errors)
    end

    it 'should delegate to #receiver' do
      allow(receiver).to receive(:update_negated_errors_for)
