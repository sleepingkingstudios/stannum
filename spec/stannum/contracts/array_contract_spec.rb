...
RSpec.describe Stannum::Contracts::ArrayContract do
  ...
  shared_context 'when initialized with item_type: value' do
    let(:constructor_options) { { item_type: String } }
  end
  ...
  let(:expected_options)    { { allow_extra_items: false, item_type: nil } }
  ...
  describe '::Builder' do
    ...
    let(:contract) do
      Stannum::Contracts::ArrayContract.new # rubocop:disable RSpec/DescribedClass
    end
  ...
  describe '.new' do
    it 'should define the constructor' do
      expect(described_class)
        .to be_constructible
        .with(0).arguments
        .and_keywords(:allow_extra_items, :item_type)
        .and_any_keywords
        .and_a_block
    end
  ...
  describe '#each_constraint' do
      ...
      [
        an_instance_of(Stannum::Contracts::Definition).and(
          satisfy do |definition|
            ...
            expect(definition.constraint.item_type).to be nil
    ...
    wrap_context 'when initialized with item_type: value' do
      let(:builtin_definitions) do
        [
          an_instance_of(Stannum::Contracts::Definition).and(
            satisfy do |definition|
              expect(definition.constraint)
                .to be_a Stannum::Constraints::Types::ArrayType
              expect(definition.constraint.item_type)
                .to be_a Stannum::Constraints::Type
              expect(definition.constraint.item_type.expected_type)
                .to be String
              expect(definition.sanity?).to be true
            end
          ),
  ...
  describe '#item_type' do
    ...
    context 'when initialized with item_type: nil' do
      ...
      it { expect(contract.item_type).to be nil }
    end
    ...
    context 'when initialized with item_type: a constraint' do
      ...
      it { expect(contract.item_type.options).to be == item_type.options }
    end
  end
  ...
  describe '#options' do
    let(:expected) do
      { allow_extra_items: false, item_type: nil }
    end
    ...
    wrap_context 'when initialized with allow_extra_items: true' do
      it 'should set the option' do
        expect(contract.options)
          .to be == expected.merge(allow_extra_items: true)
      end
    end
    ...
    wrap_context 'when initialized with item_type: value' do
      it 'should set the option' do
        expect(contract.options)
          .to be == expected.merge(item_type: String)
      end
    end
  end
  ...
  describe '#with_options' do
    ...
    describe 'with item_type: value' do
      let(:error_message) { "can't change option :item_type" }

      it 'should raise an exception' do
        expect { contract.with_options(item_type: true) }
          .to raise_error ArgumentError, error_message
      end
    end
  end
